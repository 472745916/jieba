...
def getKeywords_textrank(data, topK):
    ...
    for index in range(len(idList)):
        ...
        jieba.load_userdict("/Users/gyq/Desktop/oo/dict.txt")  # 词典

        data = jieba.cut(text, cut_all=False, HMM=True)  # 分词
        data_split = "/".join(data)

        keywords = jieba.analyse.extract_tags(data_split, topK=topK, allowPOS=())  # TextRank关键词提取，词性筛选
